...
<div class="bg-white rounded-xl shadow-sm p-8 border-2 border-[#dfdfdf]">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            <h2 class="text-2xl font-semibold text-gray-800">Edit Warranty</h2>
            <p class="text-gray-500 text-sm">Update details for the selected warranty.</p>
        </div>
    ...
    {{-- Form --}}
    <form action="{{ route('backend.warranties.update', $warranty->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <div class="flex gap-3">
                    <input type="number" id="duration" name="duration"
                        value="{{ old('duration', $warranty->duration) }}"
                    ...
                    <select id="duration_type" name="duration_type"
                        ...
                        <option value="days" {{ old('duration_type', $warranty->duration_type) == 'days' ? 'selected' : '' }}>Days</option>
                        <option value="months" {{ old('duration_type', $warranty->duration_type) == 'months' ? 'selected' : '' }}>Months</option>
                        <option value="years" {{ old('duration_type', $warranty->duration_type) == 'years' ? 'selected' : '' }}>Years</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <textarea id="description" name="description" rows="4"
                ...
                placeholder="Write a short description...">{{ old('description', $warranty->description) }}</textarea>
        </div>
        ...
        <div class="flex items-center space-x-2">
            ...
            <input type="checkbox" id="status" name="status" value="1"
                ...
                {{ old('status', $warranty->status) ? 'checked' : '' }}>
        ...
        <div class="flex justify-end space-x-3 pt-4">
            ...
            <button type="submit"
                ...
                <i class="fas fa-save mr-2"></i> Update Warranty
            </button>
        </div>
    </form>
</div>
